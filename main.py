...
con = sqlite3.connect('bomber.db')
cur = con.cursor()
sound = pygame.mixer.Sound("sounds/Red_Hot_Chilli_Peppers_-_Give_It_Away_8-BIT.mp3")
...
# Размер одной клетки, словарь для картинок
...
class BomberMan(pygame.sprite.Sprite):
    ...
    # Функции update в подобных классах служат для анимации спрайта
    ...
    # Функция stop прекращает проигрывание анимации, когда игрок неподвижен
    ...
    def walking(self):
        ...
        # Функция, ответственная за передвижения спрайта главного героя
...
class Enemy(pygame.sprite.Sprite):
    ...
    # Функция get_direction есть только в классе врагов, она отвечает за самостоятельные передвижения по карте
...
# Класс лучка - вида врагов
# Наследуется от класса Enemy
class Onion(Enemy):
    def __init__(self, x, y):
        super().__init__(x, y)
        self.velocity = 1 / 4
...
def level_animating():
    ...
    inscription.rect.x = -500
    inscription.rect.y = 125
    while inscription.rect.x != 100:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                terminate()
        screen.fill((0, 0, 0))
...
def updating_amount():
    ...
    inscription_sprite = pygame.sprite.Group()
    inscription = pygame.sprite.Sprite(inscription_sprite)
    inscription.image = pygame.transform.scale(load_image("left_inscription.png"), (170, 30))
    inscription.rect = inscription.image.get_rect()
    inscription.rect.x = 8
    inscription.rect.y = 14
    ...
    for x in nums:
        ...
        inscription.rect.y = 5
        x_n += 40
    inscriptiontime = pygame.sprite.Sprite(inscription_sprite)
    inscriptiontime.image = pygame.transform.scale(load_image("time_ins.png"), (170, 30))
    inscriptiontime.rect = inscriptiontime.image.get_rect()
    inscriptiontime.rect.x = 400
    inscriptiontime.rect.y = 14


scores_sprite = pygame.sprite.Group()


# Счёт, его вывод в конце, добавление в базу данных

def scores():
    global scores_sprite
    scores_sprite = pygame.sprite.Group()
    final_score = pygame.sprite.Sprite(scores_sprite)
    final_score.image = pygame.transform.scale(load_image("score_inscription.png"), (200, 40))
    final_score.rect = final_score.image.get_rect()
    final_score.rect.x = 8
    final_score.rect.y = 8
    sc = str(score)
    data = datetime.datetime.now()
    res = cur.execute('INSERT INTO records (data, score) VALUES(?, ?)', (data, score))
    con.commit()
    x_c = 230
    for x in sc:
        final_score = pygame.sprite.Sprite(scores_sprite)
        final_score.image = pygame.transform.scale(load_image(x + ".png"), (30, 40))
        final_score.rect = final_score.image.get_rect()
        final_score.rect.x = x_c
        final_score.rect.y = 8
        x_c += 40
...
if __name__ == '__main__':
    ...
    while running:
        ...
        # Проверка пересечения спрайта игрока и моба
        ...
        # Проверка столкновения спрайтов с ветвями взрыва
